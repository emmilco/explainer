...
export function bakeFields(region, step = 1, mod = null) {
  const nx = Math.round((region.x1 - region.x0) / step) + 1, nz = Math.round((region.z1 - region.z0) / step) + 1;
  const hts = new Float32Array(nx * nz), mask = new Uint8Array(nx * nz * 4);
  for (let j = 0; j < nz; j++) {
    const z = region.z0 + j * step;
    for (let i = 0; i < nx; i++) {
      const x = region.x0 + i * step, k = j * nx + i;
      hts[k] = T.height(x, z);
      let [lush, dry] = T.grassAt(x, z);
      let tall = 0;
      if (mod) { const [t, sup] = mod(x, z); tall = t; lush = Math.max(lush * (1 - sup), t); dry *= 1 - sup; }
      mask[k * 4] = Math.round(lush * 255); mask[k * 4 + 1] = Math.round(dry * 255); mask[k * 4 + 2] = Math.round(tall * 255);
    }
  }
  const hTex = new THREE.DataTexture(hts, nx, nz, THREE.RedFormat, THREE.FloatType);
  hTex.minFilter = hTex.magFilter = THREE.NearestFilter; hTex.needsUpdate = true;
  const mTex = new THREE.DataTexture(mask, nx, nz, THREE.RGBAFormat);
  mTex.minFilter = mTex.magFilter = THREE.LinearFilter; mTex.needsUpdate = true;
  return { hTex, mTex, origin: new THREE.Vector2(region.x0, region.z0), size: new THREE.Vector2((nx - 1) * step, (nz - 1) * step), step };
}
...
export function makeGrass({ fields, sunDir, sunCol, rnd, patches }) {
  ...
  return { meshes, update(cam, time) { shared.uCam.value.set(cam.x, cam.z); shared.uTime.value = time; } };
}
