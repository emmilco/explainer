...
export function build(scene, vigs, deps) {
  ...
  if (blobs.length) {
    ...
    const p = g.attributes.position, col = new Float32Array(p.count * 3), base = srgb(0.13, 0.22, 0.07);
    for (let i = 0; i < p.count; i++) {
      ...
      v3.multiplyScalar(1 + n1 * 0.06 + n2 * 0.05);
    ...
    const mesh = new THREE.InstancedMesh(g, new THREE.MeshStandardMaterial({ vertexColors: true, roughness: 0.95 }), blobs.length);
